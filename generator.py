...
def maze_generator(traceback, current_pos, cells, screen, grid_size_x, grid_size_y):
    ...
    if len(neighbours) > 0:
        ...
        if (current_x, current_y) in cells:
            cells[(current_x, current_y)].make_path(screen)
    ...
    #temp solution
        ...
        traceback += 1
        try:
            traceback %= len(traceback_stack)
        except:
            return None, None
        else:
            if debug: print(f"tb {traceback} % {len(traceback_stack)} = {traceback % len(traceback_stack)}")
            traceback %= len(traceback_stack)
            current_pos = traceback_stack[-traceback]
            traceback_stack.remove(current_pos)

    return current_pos, traceback
